...
> ---
> **⚠️ 平行版本提示（合并进 `ipad-cfd-teaching/main` 时补记）**
> 本文件是 ③ 阶段的**另一条会话线**（2026-06-23，「Phase 3+ — Architecture Design Recording」）产出的 v0.3，原在分支 `claude/wonderful-ride-ds1ee5` 上叫 `architecture.md`。
> 仓库当前的 `architecture.md` 是**另一版 v0.3**（2026-06-13，「Phase 4 — 详细设计」会话线），`detailed-design.md` 与 `HANDOFF-stage4/5.md` 建立在那一版之上、**未参考本文件**。
> 保留本文件是为了不丢失本线独有的内容：更详细的 §0、§6.1 协作视图（SolverEvent 流）、§13 架构评审记录，以及 `adr/ADR-001..012` 这套独立 ADR 文件。
> 两版差异待人工裁决后再收敛为单一 v0.4。
> ---
