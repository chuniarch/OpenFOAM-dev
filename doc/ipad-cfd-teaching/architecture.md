...
> 版本：**v0.3（已冻结 · ③架构设计）**　|　日期：2026-06-13（v0.2 定稿 2026-06-10）
> 修订史：v0.3 — ③架构设计完成并冻结：§13 ADR 台账（ADR-001..011）、§14 D1–D4 裁决、§15 引擎↔联动层事件接口（补 §1.1 缺口）、§16 资源层契约升级、§17 阶段末评审记录（场景走查 + 断言核实）。遗留 1 个 ④ 待办（UC5 矩阵行访问器）+ D1 暂定项，均不阻塞。
> 评审状态：**③架构设计 v0.3 已冻结**（评审记录见 §17）。§2 类映射 / §4 案例数据（§2.1、§4.5）、§13 ADR、§15 事件接口、§16 资源契约 均经核实/走查。下一阶段：④ 详细设计。
> 定位：**OpenFOAM 的「引桥课」**——在 iPad 上零环境成本走通一次「从输入到流场」的完整闭环，并直接对照真实 OpenFOAM 源码，让初学者带着心智模型去面对真正的工具。
> ---
> **⚠️ 平行版本提示（合并 `claude/wonderful-ride-ds1ee5` 时补记）**
> ③ 阶段被两个会话各自冻结过一次，产生两份 v0.3：
> - **本文件**（2026-06-13，「Phase 4 — 详细设计」会话线）：ADR 台账**内联**在 §13，并继续写到 §14–§17。`detailed-design.md`、`HANDOFF-stage4/5.md` 全部建立在本文件之上。
> - **`architecture-v0.3-adr-line.md`**（2026-06-23，「Phase 3+ — Architecture Design Recording」会话线）：把 ADR **拆成 12 份独立文件**（见 `adr/`），§13 改为架构评审记录，§0 更详细，新增 §6.1 协作视图。
>
> 两版在 §0–§12 各自有改动，**未做正文合并**——④ 详细设计只见过本文件。两版差异待人工裁决后再收敛为单一 v0.4。
> ---
...
> 范围：本批 **ADR-001..007 = §0 的 7 条已定决策**。另有 3 条架构显著决策（分层 + 单一事实源 / GPL 策略 / 图形化编辑）已于 v0.3 冻结补为 ADR-008..010（见下）；引擎事件接口为 ADR-011（§15）。
...
- **状态**：已接受（正规化自 §1、§6）。
- **背景**：FR4/FR5「点一处 → 多面板响应」要求多面板**一致刷新**；NFR4（流畅）；ADR-001（引擎可单测、零 UI）。
- **决策**：分四层——展示层(SwiftUI+Metal) / 联动层(@Observable，持**单一事实源** `highlightedNodeID + iterationState + probedCell`) / 引擎层(纯计算零 UI) / 资源层(内嵌数据)。依赖方向**自上而下单向**：展示→联动→(引擎+资源)；引擎/资源不知上层存在。
- **落选选项（及否决它的需求）**：
  - 各面板各自持状态、互相直接调用 —— 被「一致性」否决（多份状态必漂移，FR4 多面板联动会对不上）。
  - 引擎直接驱动 UI（引擎里调刷新）—— 被 **ADR-001（零 UI、可单测）** 否决。
- **后果与缓解**：正面 = 一致性由单一事实源保证、各层可独立替换/测试、§15 事件流天然汇入此源。负面 = 多一层间接。缓解 = 教学 App 规模小，间接成本可忽略。
- **追溯**：FR4 / FR5 / NFR4；§1、§6；关联 ADR-001、ADR-011。

### ADR-009 · GPL 策略：原型期内嵌、上架前切「外链 + 自写等价讲解」
- **状态**：已接受（正规化自 §11.4；完整分析见 §11）。
- **背景**：C3（OpenFOAM GPL v3 与 App Store 分发条款冲突，著名坑）；ADR-002（展示真实源码 = 复制 GPL 文本）。
- **决策**：原型/开发期先内嵌真实源码、不为合规分心；**上架前**改造为 ①源码外链 openfoam.org + ②源码面板用自写等价讲解，并确认整 App 分发许可。
- **落选选项（及否决它的需求）**：见 §11.3 五选项。纯内嵌直接上架 —— 被 **C3** 否决；放弃 App Store 走自签/TestFlight —— 被商业化目标否决（非硬需求，留作退路③）；故选 ①+② 组合（不复制 GPL 文本、仍可对照真实 OpenFOAM、可上架）。
- **后果与缓解**：正面 = 原型期不被合规拖慢、改造与引擎/可视化解耦、后期替换成本可控。负面 = 上架前必须做这次切换（债务）。缓解 = §11.4 列为上架前待办、源码片段保留许可头。
- **追溯**：C3；ADR-002；§11。

### ADR-010 · 图形化编辑：表单 ↔ dict 文本 双向同步
- **状态**：已接受（正规化自 §6 面板⑦）。
- **背景**：NFR3（无键盘也能完成全部输入 → 须图形化）；C1（用户只能编辑数据）；FR2（编辑输入、同时可见等价 dict 文本）。
- **决策**：编辑用图形控件（滑杆/选择器/步进器），同时显示等价 OpenFOAM dict 文本，二者**双向同步**；编辑作用于结构化 `CaseData`（ADR-005）。
- **落选选项（及否决它的需求）**：
  - 纯文本编辑 dict —— 被 **NFR3**（触屏手敲字典体验差）否决。
  - 纯表单、不显示 dict 文本 —— 被 **FR2** 否决（须可见等价 dict 文本，这本身是教学法：让用户认识真实字典格式）。
- **后果与缓解**：正面 = 既解触屏体验又教真实字典格式（一举两得）。负面 = 双向同步 + 文本侧校验（UC2-E2 非法字典行级标错）须实现。缓解 = MVP 仅 4 种 BC、参数有限、安全范围钳制（NFR2/C4）。
- **追溯**：NFR3 / C1 / FR2；§6 面板⑦；关联 ADR-005。

> 台账小结：ADR-001..003 是「源码入门」差异化的三连扣（Swift 跑 + 真 C++ 看 + 映射表锁一致）；ADR-004/005 锚定范围与可演进性；ADR-006/007 是呈现层选型；ADR-008 是结构骨架（分层 + 单一事实源）、ADR-009 是 GPL 策略、ADR-010 是编辑范式、ADR-011（§15）是引擎事件接口。**落选选项几乎全由 NFR 与约束（C1–C4、NFR1–NFR6）否决，FR 极少直接否决某选项**——再次印证「需求→架构 ASR 筛法」：NFR/约束才是架构显著性的主力。
