...
> 修订史:v0.1（2026-06-12 首切:用例清单 + 2 条详例 + 领域模型草图）→ **v0.2（2026-06-12 精炼:8 条用例全部补齐异常流;领域模型补基数与组合/引用;新增源码关系图数据格式草案;新增用例→验收测试派生首批;新增开放决策点清单）** → **v0.3（2026-06-15 新增 D5：FR7 AI 后端选型 + 可替换接口 + 端侧 RAG-over-本地内容）**。
...
| # | 决策点 | 候选 | 归属 |
|---|---|---|---|
| D1 | UC6 的 detail 呈现 | 浮层 vs 固定侧栏(一区或两区) | ③ UX 架构 |
| D2 | 单步粒度 | 仅时间步 vs 时间步+PISO 子步两档 | ③(影响引擎事件接口) |
| D3 | 编辑后重算策略 | 重置重算(MVP) vs 从当前时刻续算 | ③/④ |
| D4 | 图布局算法 | 手工定坐标(MVP,节点少) vs 自动布局 | ③ 资源层/渲染 |
| D5 | FR7 AI 后端选型 | ①云端前沿模型 ②Apple 自带模型(Foundation Models) ③Core AI 端侧自带小模型 + RAG ④混合 | ③ 立接口 + M5 选型 |
